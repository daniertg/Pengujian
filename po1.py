def get_java_to_python_prompt_beginner(java_code: str, rag_context: str = "") -> str:   
    context_section = ""
    if rag_context:
        context_section = f"""
REFERENCE EXAMPLES (Use these as guidance for similar patterns):
{rag_context}

Based on the reference examples above, """
...
a. Java = <Java code>
   Python = <equivalent Python code if not exists answer "Tidak ada syntax yang setara">
...
- Object creation:
  Mahasiswa mhs = new Mahasiswa("..", ...);  # will be rejected
- Exception handling:
...
def get_java_to_python_prompt_expert(java_code: str, rag_context: str = "") -> str:    
    context_section = ""
    if rag_context:
        context_section = f"""
REFERENCE EXAMPLES (Use these as guidance for similar patterns):
...
Based on the reference examples above, """
    ...
    return f"""
{context_section}After checking the rules and output format, convert the following Java code to Python that is fully compatible with Skulpt (Python-in-the-browser):
RULES:
